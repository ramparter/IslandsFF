...
public class GameManager : GooglePlayGames.BasicApi.OnStateLoadedListener {
    ...
    private void UnlockProgressBasedAchievements() {
        int totalStars = mProgress.LevelsCompleted;
        int i;
    ...
    public void PostToLeaderboard() {
        long distance = mProgress.LongestDistance;
        if (Authenticated && distance > mLongestPostedDistance)
        {
            // post score to the leaderboard
            Social.ReportScore(distance, GameIds.LeaderboardId, (bool success) => { });
            mLongestPostedDistance = distance;
        }
    }
}
